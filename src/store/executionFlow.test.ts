...
describe('Quokka-like Execution Flow', () => {
    ...
    it('should update executionErrors when receiving execution:error messages', async () => {
        ...
        await vi.waitFor(() => {
            ...
            // executionSlice wraps error messages in an object
            expect(errors.get(5)).toMatchObject({ message: 'Something went wrong', line: 5, column: 10 });
        });
    });
    ...
    it('should clear previous results on new execution', async () => {
        ...
        // Run different code to force re-execution
        runExecution('const a = 20;');

        // Simulate backend confirming execution started
