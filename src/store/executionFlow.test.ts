...
describe('Quokka-like Execution Flow', () => {
    ...
    beforeEach(async () => {
        // Reset modules to reset `listenersInitialized` in executionSlice
        vi.resetModules();

        // Re-import useStore after reset
    ...
    it('should send execution request to backend', async () => {
        const { useStore } = await import('./useStore');
    ...
    it('should update executionErrors when receiving execution:error messages', async () => {
        ...
        await vi.waitFor(() => {
            ...
            expect(errors.get(5)).toEqual({ message: 'Something went wrong', line: 5, column: 10 });
        });
    });
    ...
    it('should clear previous results on new execution', async () => {
        ...
        await vi.waitFor(() => expect(startCallback).toBeDefined());
        if (startCallback) startCallback(); // Backend says started
        ...
        // Run different code to bypass 'same code' check
        runExecution('const a = 20;');

        // Manually trigger clear callback as backend would
